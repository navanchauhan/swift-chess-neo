...
public struct PGNMove: RawRepresentable, ExpressibleByStringLiteral {
  ...
  static private let pattern =
    "^(?:([NBRQK]?)([a-h]?)([1-8]?)(x?)([a-h])([1-8])((?:=[NBRQ])?)|(O-O)|(O-O-O))([+#]?)$"
  static private let regex: NSRegularExpression! = try? NSRegularExpression(
  ...
  public var rank: Rank {
    ...
    guard let raw = Int(rankSymbol), let rank = Rank(rawValue: raw) else {
  ...
  public var sourceRank: Rank? {
    ...
    guard let sourceRankRaw = Int(sourceRankSymbol),
      let sourceRank = Rank(rawValue: sourceRankRaw)
    else { fatalError("Could not get source rank") }
    return sourceRank
  }
  ...
  public var sourceFile: File? {
    ...
    guard let sourceFileRaw = sourceFileSymbol.first,
      let sourceFile = File(sourceFileRaw)
    else { fatalError("Could not get source file") }
    return sourceFile
  }
  ...
  public var promotionPiece: Piece.Kind? {
    ...
    guard let pieceLetter = stringAt(rangeIndex: 7).last,
      let piece = PGNMove.pieceFor(letter: String(pieceLetter))
    else { fatalError("Could not get promotion piece") }
    return piece
  }
